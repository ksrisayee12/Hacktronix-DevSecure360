...
### 3 Confirmed Bugs Still Present (Fix These First — See Section 5)

**Bug 1:** `js_cookie_security.yaml` has `confidence: Confirmed`
Should be `confidence: Tentative` — cookie flag detection is structural/absence-based.

**Bug 2:** These 4 files have `vuln_class: Type Confusion` — wrong class:
`csharp_insecure_reflection.yaml`, `java_insecure_reflection.yaml`,
`php_insecure_reflection.yaml`, `python_insecure_reflection.yaml`
Should be `vuln_class: Code Injection` — reflection with user input = code execution.

**Bug 3:** These 4 files have `vuln_class: Log Forging` — not in allowed list:
`csharp_log_injection.yaml`, `go_log_injection.yaml`,
`js_log_injection.yaml`, `php_log_injection.yaml`
Should be `vuln_class: Log Injection` — that is the correct allowed value.
